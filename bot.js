...
client.channels.get(`الايدي بتاع الروم ال\ي سيتم عرض السبب به`).sendMessage("** تم طرد هذا الشخص من قبل " + message.guild.owner + " سبب مذكور **" + args.join("  "))
        }).catch(() => {
...
const Sra7a = [ //لاست كودز
  ...
  client.on('message', message => { //لاست كودز
if (message.content.startsWith('صراحة')) { //لاست كودز
    if(!message.channel.guild) return message.reply('** This command only for servers **'); //لاست كودز
 var client= new Discord.RichEmbed() //لاست كودز
 .setTitle("لعبة صراحة ..") //لاست كودز
 .setColor('RANDOM') //لاست كودز
 .setDescription(`${Sra7a[Math.floor(Math.random() * Sra7a.length)]}`) //لاست كودز
 .setImage("https://cdn.discordapp.com/attachments/371269161470525444/384103927060234242/125.png") //لاست كودز
                 .setTimestamp() //لاست كودز
 
  message.channel.sendEmbed(client); //لاست كودز
